...
function getUppercaseNames(items) {
  // TODO: use `map`
  const namesUpperCase = items.map((item)=> item.name.toUpperCase());
...
function getItemPriceByName(items, name) {
  // TODO: use a loop!
  //const priceByName = 
  for (let i = 0; i < getItemPriceByName.length; i++) {
    for (let j = 0; j < getItemPriceByName[i].length; j++)
    console.log(getItemPriceByName[i][j]);
    return getItemPriceByName;
  }
}
...
function getItemsByCategory(items, category) {
  // TODO: use `filter`
  const itemsByCategory = items.filter(item=> item.category.length - 1 === category);
...
function countItems(items) {
  ...
  const itemsCount = items.flat().reduce((acc, item) => {
    if (acc[item.name]) {
    ...
    //(sum: number, {node: {quantity}}: items) => sum + quantity, 0)
      //return sum + quantity;
    }
...
function calculateTotalPrice(items) {
  // TODO: use `reduce`
  const itemsCost = items.flat().reduce((acc, item) => {
    const allItemsCost = item.quantity * item.price;
    return acc + allItemsCost;
  }, 0);
}
